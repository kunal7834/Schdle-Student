...
// ---------------- Calendar View ----------------
const CalendarView = ({ currentMonth, setCurrentMonth, selectedDate, setSelectedDate, getEventsForDate, viewMode, setViewMode }) => {
...
// ---------------- Student Login ----------------
const StudentLogin = ({ onLogin, rosters }) => {
      ...
      for (const s of (rosters[sec] || [])) {
        if (String(s.email).trim().toLowerCase() === wantedEmail) {
          ...
          if (fn.toLowerCase() === wantedPwd.toLowerCase()) {
            setError('');
            onLogin({ name: s.name, email: s.email, rollNumber: s.rollNumber, section: sec });
            return;
          }
        }
      }
...
const StudentPlatform = () => {
  ...
  const [session, setSession] = useState(null); // { name, email, rollNumber, section }
  ...
  if (!session) {
    return <StudentLogin onLogin={setSession} rosters={rosters} />;
  }

  const myEvents = events.filter(e => (e.sections || []).includes(session.section));
      ...
      <div className="bg-white border-b border-slate-200 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto flex justify-between items-center px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-base font-semibold text-slate-900 tracking-tight">Student Portal</h1>
              <p className="text-xs text-slate-500" data-testid="student-current-user">
                {session.name} · {session.email} · Sec {session.section}
              </p>
            </div>
          </div>
          <button data-testid="student-logout-btn" onClick={logout} className="bg-slate-900 hover:bg-slate-700 text-white px-4 py-2 rounded-lg transition-colors text-sm font-medium">Logout</button>
        </div>
      </div>
